...
//Define bar chart function 
		...
		svg.selectAll("rect")
		    ...
		    .on('mouseover', function(d){
					...
					if(state[0][0].selected){
						info.append("p")
							    .text(formatPercent(d.postSentiment));
		...
		//Change data to correct values on input change
			...
			on("change", function() {
				...
				if(value=="bus"){
					...
					var x_value = function(d){return d.postSentiment;};
					var color = function(d){return d.postSentiment < 0 ? "negative" : "positive";};
					var y_value = function(d){
			    		return yScale(Math.max(0, d.postSentiment)); 
			    	};
			    	var height_value = function(d){
	...
	//Load data and call bar chart function 
		d3.json("fbPageData.json", function(error, data){
